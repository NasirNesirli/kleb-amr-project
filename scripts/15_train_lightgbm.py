...
def main():
    ...
    if len(unique_classes) == 1:
        ...
        dummy_results = {
            'cv_results': [{
                ...
                'best_params': {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.1, 'num_leaves': 31}
            }],
            'test_results': {
                'f1': 0.0 if single_class == 0 else 1.0,
    ...
    location_year_train = (train_merged['Location'].fillna('unknown').astype(str) + '_' + 
                          train_merged['Year'].fillna('unknown').astype(str)).values
    ...
    # Train final model with class weighting
    ...
    plt.xlabel('CV Fold')
    plt.ylabel('F1 Score')
    plt.title(f'LightGBM CV F1 Scores\n{antibiotic}')
    plt.ylim(0, 1)
